...
class ExcelStoreBase(AbstractStore):
    ...
    def __init__(self, platform: str, crawler_type: str = "search"):
        ...
        # Track stored IDs for deduplication
        self.stored_content_ids = set()
        self.stored_comment_ids = set()
        self.stored_creator_ids = set()
    ...
    async def store_content(self, content_item: Dict):
        """
        Store content data to Excel with automatic deduplication

        Args:
            content_item: Content data dictionary
        """
        # Get ID from various possible field names for deduplication
        content_id = content_item.get('note_id') or content_item.get('aweme_id') or content_item.get('video_id') or content_item.get('content_id')
        
        # Check for duplicates
        if content_id and content_id in self.stored_content_ids:
            utils.logger.debug(f"[ExcelStoreBase] Skipped duplicate content: {content_id}")
            return
        ...
        # Add to stored IDs set
        if content_id:
            self.stored_content_ids.add(content_id)

        content_id_display = content_id or 'N/A'
        utils.logger.info(f"[ExcelStoreBase] Stored content to Excel: {content_id_display}")
    ...
    async def store_comment(self, comment_item: Dict):
        ...
        # Get comment ID for deduplication
        comment_id = comment_item.get('comment_id')
        
        # Check for duplicates
        if comment_id and comment_id in self.stored_comment_ids:
            utils.logger.debug(f"[ExcelStoreBase] Skipped duplicate comment: {comment_id}")
            return
        ...
        # Add to stored IDs set
        if comment_id:
            self.stored_comment_ids.add(comment_id)
    ...
    async def store_creator(self, creator: Dict):
        ...
        # Get creator ID for deduplication
        creator_id = creator.get('user_id') or creator.get('creator_id')
        
        # Check for duplicates
        if creator_id and creator_id in self.stored_creator_ids:
            utils.logger.debug(f"[ExcelStoreBase] Skipped duplicate creator: {creator_id}")
            return
        ...
        # Add to stored IDs set
        if creator_id:
            self.stored_creator_ids.add(creator_id)
