...
class XiaoHongShuClient(AbstractApiClient, ProxyRefreshMixin):
    ...
    async def request(self, method, url, **kwargs) -> Union[str, Any]:
        ...
        try:
            ...
            utils.logger.error(f"[XiaoHongShuClient.request] JSON解析失败: {response.text[:200]}")
            raise DataFetchError(f"JSON解析失败: {str(e)}")
        
        # 安全访问字段，避免KeyError
        if "success" in data and data["success"]:
            return data.get("data", data.get("success", {}))
        elif "code" in data and data.get("code") == self.IP_ERROR_CODE:
            raise IPBlockError(self.IP_ERROR_STR)
        elif "code" in data or "msg" in data:
            err_msg = data.get("msg", None) or data.get("message", None) or f"{response.text[:200]}"
            raise DataFetchError(err_msg)
        else:
            # 如果响应格式不符合预期，返回整个响应或空字典
            utils.logger.warning(f"[XiaoHongShuClient.request] 意外的响应格式: {list(data.keys())}")
        ...
        while comments_has_more and len(result) < max_count:
            try:
                ...
                if not comments_res or not isinstance(comments_res, dict):
                    utils.logger.warning(f"[XiaoHongShuClient.get_note_all_comments] 评论响应为空或格式错误: {note_id}")
                    break
                ...
                if "comments" not in comments_res:
                    utils.logger.info(
                        f"[XiaoHongShuClient.get_note_all_comments] No 'comments' key found in response: {note_id}"
                    )
                    break
                comments = comments_res["comments"]
            except (KeyError, DataFetchError, Exception) as e:
                utils.logger.error(f"[XiaoHongShuClient.get_note_all_comments] 获取评论失败 {note_id}: {str(e)}")
                break
