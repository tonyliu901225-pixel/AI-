...
async def parse_cmd(argv: Optional[Sequence[str]] = None):
    ...
    def main(
        ...
        max_count: Annotated[
            ...
            typer.Option(
                "--max_count",
                help="Maximum number of posts/videos to crawl (default: 150)",
                rich_help_panel="Basic Configuration",
            ),
        ...
        sort_type: Annotated[
            ...
            typer.Option(
                "--sort_type",
                help="Sort type for search results (popularity_descending=点赞数排序 | comment_descending=评论数排序 | time_descending=时间维度排序)",
                rich_help_panel="Basic Configuration",
            ),
        ] = "",
    ) -> SimpleNamespace:
        ...
        # CRITICAL FIX: Always update keywords from command line parameter
        # Typer will pass the value from --keywords if provided, or default (config.KEYWORDS) if not
        # We always use the parameter value, stripping whitespace
        # Note: If --keywords "" (empty) is passed, keywords will be empty string
        original_keywords = getattr(config, '_original_keywords', config.KEYWORDS)
        if not hasattr(config, '_original_keywords'):
            config._original_keywords = config.KEYWORDS
        # CRITICAL FIX: Update keywords from command line parameter
        # If --keywords was provided and is non-empty, use it
        # If --keywords was not provided, Typer uses default (config.KEYWORDS), so we keep that
        # If --keywords "" (empty) was provided, keep original default (empty doesn't make sense)
        import logging
        logger = logging.getLogger(__name__)
        
        # CRITICAL: Log BEFORE processing to see what Typer passed
        logger.info(f"[cmd_arg] 🔍 Received keywords parameter: {repr(keywords)} (type: {type(keywords).__name__})")
        logger.info(f"[cmd_arg] 🔍 Original default keywords: {repr(original_keywords)}")
        logger.info(f"[cmd_arg] 🔍 Current config.KEYWORDS before update: {repr(config.KEYWORDS)}")
        
        if keywords is not None:
            if isinstance(keywords, str):
                keywords_stripped = keywords.strip()
                if keywords_stripped:
                    # Non-empty keywords: use them
                    config.KEYWORDS = keywords_stripped
                    logger.info(f"[cmd_arg] ✅ Using keywords from command line: {repr(keywords_stripped)} (length: {len(keywords_stripped)})")
                else:
                    # Empty keywords: keep original default
                    logger.warning(f"[cmd_arg] ⚠️ Empty keywords provided, keeping default: {repr(config._original_keywords)}")
                    config.KEYWORDS = config._original_keywords
            else:
                # Non-string: use as-is or fall back to default
                config.KEYWORDS = keywords if keywords else config._original_keywords
                logger.warning(f"[cmd_arg] ⚠️ Keywords is not a string (type: {type(keywords).__name__}), using: {repr(config.KEYWORDS)}")
        else:
            logger.warning(f"[cmd_arg] ⚠️ Keywords parameter is None, keeping default: {repr(config.KEYWORDS)}")
        
        # Log AFTER processing to confirm update
        logger.info(f"[cmd_arg] ✅ Final config.KEYWORDS after update: {repr(config.KEYWORDS)}")
        
        # Set max_count for crawling limit
        if max_count and max_count > 0:
            config.CRAWLER_MAX_NOTES_COUNT = max_count
            logger.info(f"[cmd_arg] ✅ Set CRAWLER_MAX_NOTES_COUNT to {max_count}")
        
        # Set sort_type for search sorting
        if sort_type and sort_type.strip():
            # For xhs platform, update SORT_TYPE in xhs_config
            if platform.value == "xhs":
                try:
                    from config import xhs_config
                    xhs_config.SORT_TYPE = sort_type.strip()
                    config.SORT_TYPE = sort_type.strip()
                    logger.info(f"[cmd_arg] ✅ Set SORT_TYPE to {sort_type.strip()}")
                except Exception as e:
                    logger.warning(f"[cmd_arg] ⚠️ Failed to set SORT_TYPE: {e}")
